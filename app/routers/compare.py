...
async def submit_comparison(request: Request, username1: str = Form(...), username2: str = Form(...)):
...
async def comparison_detail(request: Request, username1: str, username2: str):
    ...
    if not validation1['valid'] or not validation2['valid']:
        ...
        return templates.TemplateResponse(request=request, name="compare.html", context={"error": error_html})
